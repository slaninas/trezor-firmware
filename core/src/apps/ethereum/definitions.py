...
class EthereumDefinitionParser:
    def __init__(self, definition_bytes: bytes) -> None:
        ...
        self.format_version = int.from_bytes(definition_bytes[:2], 'big')
        self.definition_type = definition_bytes[2]
        self.data_version = int.from_bytes(definition_bytes[3:7], 'big')
        self.payload = definition_bytes[:-64]
        self.clean_payload = definition_bytes[7:-64]
...
class EthereumDefinitions:
    ...
    def __init__(
        ...
        self.token_dict: helpers.SimpleDefaultDict[bytes, TokenInfo] = helpers.SimpleDefaultDict(lambda: tokens.UNKNOWN_TOKEN)
