...
def with_keychain_from_path_and_defs(
    *patterns: str,
) -> Callable[[HandlerWithKeychainAndDefinitions[EthereumGetAddress, MsgOut]], Handler[EthereumGetAddress, MsgOut]]:
    def decorator(func: HandlerWithKeychainAndDefinitions[EthereumGetAddress, MsgOut]) -> Handler[EthereumGetAddress, MsgOut]:
        async def wrapper(ctx: wire.Context, msg: EthereumGetAddress) -> MsgOut:
            defs = definitions.get_definitions_from_msg(msg)
            schemas = _schemas_from_address_n(patterns, msg.address_n, defs.network)
            keychain = await get_keychain(ctx, CURVE, schemas)
            with keychain:
