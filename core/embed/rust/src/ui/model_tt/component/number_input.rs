...
where
    ...
    T: AsRef<str>,
{
    pub fn new(min: u32, max: u32, init_value: u32, text_func: F) -> Self {
        let text = text_func(init_value);
        Self {
            area: Rect::zero(),
            text_func,
            input: NumberInput::new(min, max, init_value).into_child(),
            paragraphs: Paragraphs::new()
                .add::<theme::TTDefaultText>(theme::FONT_NORMAL, text)
                .into_child(),
